...
import eblast.crypto.KeyLengthException;
...
public class SymmetricStreamEncodeDecodeTest {
    ...
    @Test
    public void testEncryptingDecryptingStreamsSingleLine() throws IOException, KeyLengthException {
        for (int i = 4; i < 11; i++) {
            int keyLength = 1 << i;
            System.out.println("Testing with symmetric key of length (bytes): " + keyLength);
            KeyPair keygen = KeyGenerator.generateXORKeyPair(keyLength);
            ...
            OutputStream out = new CryptoOutputStream(buffer, keygen);
            ...
            InputStream in = new CryptoInputStream(new ByteArrayInputStream(buffer.toByteArray()), keygen);
    ...
    @Test
    public void testEncryptingDecryptingStreamsMultiLine() throws IOException, KeyLengthException {
        for (int i = 4; i < 11; i++) {
            int keyLength = 1 << i;
            KeyPair keygen = KeyGenerator.generateXORKeyPair(keyLength);
