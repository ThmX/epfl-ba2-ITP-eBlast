...
public class BDecoder {
	...
	public int getNextIndicator() throws IOException {
		if (indicator == 0) {
			...
			// Used for ugly hack
			if (inSpecialMap)
				shaDigest.update((byte) indicator);
		}
		return indicator;
	}
	...
	public BEValue bdecodeMap() throws IOException {
		...
		while (c != 'e') {
			...
			// ugly hack
			boolean special = specialMap.equals(key);
			if (special)
				inSpecialMap = true;
